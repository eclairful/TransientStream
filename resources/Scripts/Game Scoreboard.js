    ...
    if (allowIntro) {
      ...
      setTimeout(() => {
        document
          ...
          .setAttribute("src", "../resources/Webms/Intro.webm");
  ...
  //now things that will happen constantly
    ...
    if (p1CharacterPrev != p1Character || p1SkinPrev != p1Skin) {
      ...
      fadeOutChara("#p1Character", "#sagaIconP1", -pMove, async () => {
        ...
        updateChar(p1Character, p1Skin, "p1Character", "sagaIconP1");
    ...
    if (bestOfPrev != bestOf) {
      if (bestOf == "Best of 5") {
        if (bestOfPrev != "Best of 3") {
          gsap.fromTo(
            "#win2P1",
            { x: -pMove },
            { x: 0, opacity: 1, ease: "power2.in", duration: fadeInTime },
          );
          gsap.fromTo(
            "#win1P1",
            { x: -pMove },
          ...
          gsap.fromTo(
            "#scoreP1",
            { opacity: 1 },
            { opacity: 0, ease: "power2.in", duration: fadeInTime },
          );
          gsap.fromTo(
            "#scoreP2",
            { opacity: 1 },
      ...
      } else if (bestOf == "Best of 3") {
        if (bestOfPrev != "Best of 5") {
          gsap.fromTo(
            "#win2P1",
            { x: -pMove },
            { x: 0, opacity: 1, ease: "power2.in", duration: fadeInTime },
...
function updateScore(pNum, pScore, pColor) {
  ...
  const scoreText = document.getElementById("scoreP" + pNum);
...
function updateBorder(bestOf) {
  document
    .getElementById("borderP1")
    .setAttribute("src", "../resources/Overlay/Border " + bestOf + ".png");
  document
    .getElementById("borderP2")
    .setAttribute("src", "../resources/Overlay/Border " + bestOf + ".png");
  bestOfPrev = bestOf;
}
...
function fadeIn(itemID) {
  gsap.to(itemID, { delay: 0.2, opacity: 1, duration: fadeInTime });
}
...
function getCharInfo(pCharacter) {
  return new Promise(function (resolve) {
    ...
    oReq.open(
      "GET",
      "../resources/Texts/Character Info/" + pCharacter + ".json",
    );
    oReq.send();
...
async function updateChar(pCharacter, pSkin, charID, sagaID) {
  ...
  charEL.setAttribute(
    "src",
    "../resources/Characters/Portraits/" + pCharacter + "/" + pSkin + ".png",
  );
  ...
  if (startup) {
    charEL.addEventListener("error", () => {
      if (charEL == document.getElementById("p1Character")) {
        charEL.setAttribute(
          "src",
          "../resources/Characters/Portraits/Random CPU.png",
        );
      } else {
        charEL.setAttribute(
          "src",
          "../resources/Characters/Portraits/Random CPU 2.png",
        );
      }
    });
  }
  ...
  sagaEL.setAttribute(
    "src",
    "../resources/Characters/Saga Icons/" + charInfo.saga + ".png",
  );
  //add a listener to show nothing if the image fails to load
  if (startup) {
    sagaEL.addEventListener("error", () => {
      showNothing(sagaEL);
    });
  }
}
